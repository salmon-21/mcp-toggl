...
// Pick transport based on --http flag. Format mirrors ms-365-mcp-server:
...
async function runHttp(host: string, port: number) {
  // Stateless: each request creates a fresh transport, like ms-365-mcp-server
  // and other read-only MCP backends. Session affinity belongs to the gateway.
  const app = express();
